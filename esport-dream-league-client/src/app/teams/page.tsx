...
function SafeImage({ src, alt, className = '', fill = false }: { 
  ...
  // If no source or source is invalid, just show the initial
  ...
  return (
    <Image
      ...
      onError={() => {
        ...
        // We don't need to do anything here as React will unmount
        // this component and render the fallback div on state change
        return false; // Return false to prevent default error image
      }}
      unoptimized // Skip Next.js image optimization to reduce complexity
    />
  );
}
      ...
      try {
        ...
        if (teamAccount) {
          ...
          if (teamAccount.roster && teamAccount.roster.length > 0) {
            const playerPromises = teamAccount.roster.map(async (position: RosterPosition) => {
              try {
            ...
            // Fetch players in batches to avoid rate limiting
            const playerDetails = [];
            const batchSize = 2;
            ...
            for (let i = 0; i < playerPromises.length; i += batchSize) {
              ...
              playerDetails.push(...results);
              
              // Add a small delay between batches
              if (i + batchSize < playerPromises.length) {
